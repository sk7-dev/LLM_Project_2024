...
min_lr = 6e-5  # Increased minimum learning rate
device = 'cuda' if torch.cuda.is_available() else 'cpu'
eval_iters = 200
n_embd = 384
n_head = 8  # Increased number of attention heads
n_layer = 8  # Increased number of layers
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads, head_size):
        ...
        torch.nn.init.normal_(self.proj.weight, mean=0.0, std=0.01)
...
def get_lr(it):
    # Longer warmup with smoother transition
    if it < warmup_iters:
        return learning_rate * (it / warmup_iters) ** 2
    # Cosine learning rate decay with improved minimum
